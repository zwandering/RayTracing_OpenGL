...
rt_triangle SceneManager::create_triangle(glm::vec3 p1, glm::vec3 p2, glm::vec3 p3, glm::vec3 n1, glm::vec3 n2, glm::vec3 n3, rt_material material)
{
...
rt_BVHNode SceneManager::create_BVHNode(int left, int right, int n, int index)
{
	rt_BVHNode BVHNode = {};
	BVHNode.left = left;
	BVHNode.right = right;
...
void SceneManager::init_buffers()
{
	...
	init_buffer(&triangleUbo, "triangles_buf", 9, scene->triangles);
	init_buffer(&BVHNodeUbo, "BVHNodes_buf", 10, scene->BVHNodes);
}
...
void SceneManager::update_buffers() const
{
	...
	update_buffer(triangleUbo, scene->triangles);
	update_buffer(BVHNodeUbo, scene->BVHNodes);

...
#define Triangle rt_triangle
#define Material rt_material
#define BVHNode  rt_BVHNode
...
#define TRIANGLE_SIZE {TRIANGLE_SIZE}
#define BVHNODE_SIZE {BVHNODE_SIZE}
...
layout( std140 ) uniform triangles_buf
{
	#if TRIANGLE_SIZE != 0
    rt_triangle triangles[TRIANGLE_SIZE];
	#else
	rt_triangle triangles[1];
	#endif
};

layout( std140 ) uniform BVHNodes_buf
{
	#if BVHNODE_SIZE != 0
    rt_BVHNode BVHNodes[BVHNODE_SIZE];
	#else
	rt_BVHNode BVHNodes[1];
	#endif
};
...
HitResult hitArray(Ray ray, int l, int r, out int num) {
    HitResult res;
    res.isHit = false;
    res.distance = INF;
	num = -1;
...
// 遍历 BVH 求交
HitResult hitBVH(Ray ray, out int num) {
    HitResult res;
    res.isHit = false;
    res.distance = INF;
    ...
    while(sp>0) {
        ...
        if(node.n>0) {
			...
			int tmp = -1;
            HitResult r = hitArray(ray, L, R, tmp);
            if(r.isHit && r.distance < res.distance){
				res = r;
				num = tmp;
			}
...
bool intersectTriangles(vec3 ro, vec3 rd, float tmin, out float t, out int num)
{
	Ray ray;
	ray.startPoint = ro;
	ray.direction = rd;
	int tmp = -1;
	HitResult hitresult = hitBVH(ray, tmp);
	t = hitresult.distance;
	num = tmp;
	bool flag = hitresult.isHit == true && hitresult.isInside == false && t>0 && t < tmin;
	return flag;
}
...
float calcInter(vec3 ro, vec3 rd, out int num, out int type)
{
	...
	int tmp;
	#if load_model
	if (intersectTriangles(ro, rd, tmin, t, tmp)){
		num = tmp; tmin = t; type = TYPE_TRIANGLE;
	}
	#endif
...
float inShadow(vec3 ro, vec3 rd, float dist)
{
	...
	int tmp;
	#if load_model
	if (intersectTriangles(ro, rd, tmin, t, tmp)) {shadow = 1;}
	#endif
	for (int i = 0; i < SPHERE_SIZE; i++)
...
// https://blog.csdn.net/yinhun2012/article/details/89913915?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522166943821216782427492969%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=166943821216782427492969&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_positive~default-1-89913915-null-null.142^v66^control,201^v3^add_ask,213^v2^t3_control1&utm_term=%E8%8F%B2%E6%B6%85%E5%B0%94%E5%8F%8D%E5%B0%84&spm=1018.2226.3001.4187
...
hit_record get_hit_info(vec3 ro, vec3 rd, vec3 pt, float t, int num, int type) {
	...
	if (type == TYPE_TRIANGLE) {
		vec3 normal = normalize(triangles[num].n1 + triangles[num].n2 + triangles[num].n3);
		hr = hit_record(triangles[num].material, normal, 0, 1);
	}
	#endif
	if (type == TYPE_SPHERE) {
...
void main()
{
	float reflectMultiplier,refractMultiplier,tm; // 反射系数，折射系数，最近碰撞距离
	vec3 col;
    rt_material mat; // 材质
	vec3 pt,n;
	vec4 ob;
	...
	vec3 mask = vec3(1.0);
	vec3 color = vec3(0.0);
	vec3 ro = vec3(scene.camera_pos); // ray_origin
	vec3 rd = getRayDir(); // ray_direciton
